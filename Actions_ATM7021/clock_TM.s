...
.equ DRAM_BASE,                 0x00008000          // DRAM Where The ANDROID! Kernel Codes Starts
.equ SRAM_BASE,                 0x01FFB800          // SRAM 0x01FFB800 As per bootloader base segment
.equ PROG_SIZE,                 0x8000              // Set this to your code size (e.g., 32KB)


.equ Injection_Start,           0x06CF200           // if you have Strings you need to add this number to its location
.equ LCD_BASE,                  0x7FB80000          // This is the base of the LCD FrameBuffer Data
...
.equ Temp_Data,                 0x31000800          // Use it for test and get values from kernel
.equ Data_BASE,                 0x40000810          // This is the base of the Read/Write Data Section
.equ cursor_x,                  0x40000600 + 0x100
.equ cursor_y,                  0x40000600 + 0x104

.equ TTBR0,                     0x00000000          // Address Where TTBR0 Remap Table Is (remap table)

//--- SOC Registers Base Addresses -----
.equ System_Control_Base,       0xB01C0000          // System Control
.equ P2WI_0_Base,               0xB0300000          // TWI_0 (Two-Wire Interface) / P2WI_0 controller
.equ P2WI_1_Base,               0xB0304000          // TWI_1 (Two-Wire Interface) / P2WI_1 controller


//--- System Control Offsets -----
.equ Chip_ID,                   0xE0                // ChipID offset

//--- P2WI Offsets -----
.equ P2WI_CTL,                  0x00                // Control
.equ P2WI_STAT,                 0x08                // Status (Busy bit #4)
.equ P2WI_DAT_IN,               0x0C                // Data Input To SOC From PMIC
.equ P2WI_DAT_OUT,              0x10                // Data Out From SOC To PMIC

//--- ATC26x Registers ----
.equ PMU_SYS_CTL2,              0x02                // System Control 2 - bit[15] check power button press/release
.equ PMU_CHARGER_CTL0,          0x39                // bit[15] 0: disable 1: enable bit[9] 1: Pre Charging Enable
.equ PMU_CHARGER_CTL1,          0x3A                // bit[15] 1: Charging end bits[0-3] Charging mA 50mA - 1500mA
.equ PMU_CHARGER_CTL2,          0x3B                // bits[4-5] PreCharge 0b00:50mA 0b01:100mA 0b10:200mA 0b11:300mA
.equ PMU_AUXADC_CTL0,           0x3E                // Default 0xFFFF this enables all ADC Channels
.equ PMU_AUXADC_CTL1,           0x3F                // Default 0b1011 set bits[0], [1], [3]

.equ PMU_BATVADC,               0x40                // bits[0-9] 0% (540) 50% (660) 100% (780)
.equ PMU_WALLVADC,              0x42                // bits[0-9] 0 = not connected to wall 0-3.7v charger voltage
.equ PMU_WALLIADC,              0x43                // bits[0-9] Wall Charger Current mA
.equ PMU_REMCONADC,             0x47                // bits[0-9] Volume +/- Vol + =>
                                                    //  if (Value < 0xE0) do vol +
                                                    //  else if (Value < 150 && Value > 0xE0) do vol -
                                                    //  else do No vol buttons preesed (idle)

.equ PMU_AuxADC0,               0x4D
.equ PMU_AuxADC1,               0x4E
.equ PMU_AuxADC2,               0x4F
.equ PMU_AuxADC3,               0x50

.equ RTC_MS,                    0x56                // RTC Minutes/Seconds - bits[0-5] Sec 0x0-0x3B bits[6-11] Min 0x0-0x3B
.equ RTC_H,                     0x57                // RTC Hours - bits[0-4] Hor 0x0-0x17
.equ RTC_DC,                    0x58                // RTC Day/Week/Century - bits[0-6] Cen 0x0-0x63 bits[9-7] WEEK_D 0x1-0x7
.equ RTC_YMD,                   0x59                // RTC Years/Months/Days - bits[0-4] Day 0x1-0x1F bits[5-8] Mon 0x1-0xC
 
                                                    // bits[9-15] Year 0x0-0x63

 .text
 .global _start
 .align 2
 _start:
    //b               skip_copy             // You Can Apply this line if you wanna run it from DRAM
                                            // it have been tested that it works Good In DRAM
                                            // DRAM is very Stable At This Bootloader Firmware
                                            // Note if you are in DRAM it would take like 15 min to init DRAM
                                            // After that you would see the clock working good.
_main:
    //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    cmp             r0, #0x000B0000         @ Are we in DRAM? (Anything Below 0x90000000)
    bhi             already_in_sram                                                                                                                                                                                                                                                                                                                                                                                 @ If PC > 0x90000000, we are already in SRAM, SKIP COPY
...
copy_loop:
    ...
    ldr             r0, =SRAM_BASE            @ Hardcoded SRAM start
...
skip_copy:
   ...
   ////>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    @=============================================================================================
    @ 4. Unlock VFP/NEON (Very Important if you Wanna Use FPU, And NEON Or it would Crash)
    ...
    @=============================================================================================
    @ 5. Specially for Coretx-A9 Speed the codes execution by enableing L1 even if no MMU
    @ This Is Essential To Stop The Flicker like the one in All winner Clock
    @ L1 Instruction Cache can be enabled without the full MMU. On many Cortex-A9 cores
    @ you can enable the I-Cache independently using CP15
    @=============================================================================================
    mrc             p15, 0, r0, c1, c0, 0   @ Read Control Register
    orr             r0, r0, #0x1000         @ Set bit 12 (I-Cache)
    ...
    // ---------------- Allow The Charging Circuit ------------------------
    bl      turnOnChargeCircuit
    // --------------------------------------------------------
...
// =============== S U B R O U T I N E =======================================
// input:
// r0: ATC26x Register index
// output:
// r0: ATC26x Register Data
// ===========================================================================
turnOnChargeCircuit:
                push            {r1-r4, lr}

                mov             r0, #PMU_CHARGER_CTL0
                bl              ATC2603C_PMIC_Register_Read

                ldr             r2, =0x8200         // Set bit[15] enable charging
                                                    // Set bit[9] enable pre-Charge phase
                orr             r1, r0, r2
                mov             r0, #PMU_CHARGER_CTL0
                bl              ATC2603C_PMIC_Register_Write

                mov             r0, #PMU_CHARGER_CTL1
                bl              ATC2603C_PMIC_Register_Read

                bic             r0, r0, #0xF        // Clear bits[0-3]
                mov             r2, #0x5            // Set bits[0-3] 0b0101:500mA
                                                    //0000:50mA     Default
                                                    //0001:100mA
                                                    //0010:200mA
                                                    //0011:300mA    Extra Safe USB Charger 1A
                                                    //0100:400mA
                                                    //0101:500mA    Safe USB Charger 2A
                                                    //0110:600mA
                                                    //0111:700mA
                                                    //1000:800mA
                                                    //1001:900mA
...
// r0: ATC26x Register Data
// ===========================================================================
ATC2603C_PMIC_Register_Read:
                push            {r1-r4, lr}
                ldr             r2, =System_Control_Base
                mov             r0, r0,LSL#20           // Clear bits[12-31] (we would return it back later)
                ldr             r1, [r2, #Chip_ID]      // Check Chip Revision
                and             r3, r1, #0xF            // Get bits[0-3] Revision_ID
                cmp             r3, #3                  // If Revision_ID = 3 so this chip has one P2WI, which is P2WI_0
                beq             .P2WI_0_Query
                ldr             r4, =P2WI_1_Base        // If Revision_ID not 3 so this chip has two P2WI's, use P2WI_1
                mov             r1, #1                  // Indicate using P2WI_1
                mov             r3, #0x1C0
                mov             r0, r0,LSR#17           // return back to the position and shift it to bit[3] required for ATC
                                                        // this is like shift it left 3
                                                        // The PIMC Register Index is ready to be called
                movt            r3, #4                  // r3 = 0x401C0
                                                        // bit[4] ready/stop 0b0 = ready
                                                        // bits[6-7] P2WI Cycles 0b11 = 3 Wait Cycle
                                                        // bit[8] P2WI mode 0b0 = Burst, 0b1 = single
                                                        // bit[18] P2WI Enable/Disable   0b0 = Disable 0b1 = Enable
                str             r3, [r4, #P2WI_CTL]
                bl              PMIC_Bus_Sync           // Start Access The Register Index which at r0
                mov             r12, #0x30              // bits[0-5] Status Word 0b110000 = Transaction Complete
                mov             r1, #1                  // Indicate using P2WI_1
                str             r12, [r4, #P2WI_STAT]
                mov             r0, #0                  // 0 = No Data Written xx = Data To Write
                bl              PMIC_Bus_Sync           // Read From The Register And Clear
                ldr             r1, [r4, #P2WI_DAT_IN]  // Get the Return Data
                ldr             r2, =0x401D0            // r2 = 0x401D0
                                                        // bit[4] ready/stop 0b1 = stop
                                                        // bits[6-7] P2WI Cycles 0b11 = 3 Wait Cycle
                                                        // bit[8] P2WI mode 0b0 = Burst, 0b1 = single
                                                        // bit[18] P2WI Enable/Disable   0b0 = Disable 0b1 = Enable
                str             r2, [r4, #P2WI_CTL]
...
.P2WI_0_Query:
                ldr             r4, =P2WI_0_Base
                mov             r1, #0                  // Indicate using P2WI_1
                mov             lr, #0x1C0
                mov             r0, r0,LSR#17
                movt            lr, #4
                str             lr, [r4, #P2WI_CTL]
                ...
                mov             r0, #0                  // 0 = No Data Written xx = Data To Write
                mov             r3, #0x30
                mov             r1, r0                  // Indicate using P2WI_1
                str             r3, [r4,#P2WI_STAT]
                bl              PMIC_Bus_Sync
                ldr             r0, [r4,#P2WI_DAT_IN]
                ldr             r12, =0x401D0
                str             r12, [r4, #P2WI_CTL]
                uxth            r0, r0
                pop             {r1-r4, pc}
// End of function PMIC_Register_Query
...
// r0: 0:success
// ===========================================================================
ATC2603C_PMIC_Register_Write:
                push            {r1-r5, lr}
                ldr             r2, =System_Control_Base
                mov             r5, R1                  // Data To Write
                ldr             R1, [R2, #Chip_ID]      // Check Chip Revision
                mvn             R0, R0,LSL#20           // Clear bits[12-31] (we would return it back later)
                and             R3, R1, #0xF            // Get bits[0-3] Revision_ID
                cmp             R3, #3                  // If Revision_ID = 3 so this chip has one P2WI, which is P2WI_0
                beq             .P2WI_0_Write
                ldr             r4, =P2WI_1_Base        // If Revision_ID not 3 so this chip has two P2WI's, use P2WI_1
                mvn             r0, R0,LSR#17           // return back to the position and shit it to bit[3] required for ATC
                                                        // The PIMC Register Index is ready to be called
                mov             r1, #1                  // Indicate using P2WI_1
                mov             r2, #0x1C0
                uxth            r0, r0
                movt            r2, #4                  // r2 = 0x401C0
                                                        // bit[4] ready/stop 0b0 = ready
                                                        // bits[6-7] P2WI Cycles 0b11 = 3 Wait Cycle
                                                        // bit[8] P2WI mode 0b0 = Burst, 0b1 = single
                                                        // bit[18] P2WI Enable/Disable   0b0 = Disable 0b1 = Enable
                str             r2, [R4, #P2WI_CTL]
                bl              PMIC_Bus_Sync           // Start Access The Register Index which at r0
                mov             r0, R5                  // r0 = Data to Be Written in the Register
                mov             r1, #1                  // Indicate using P2WI_1
                bl              PMIC_Bus_Sync           // Send the Data
                mov             r3, #0x1D0
                mov             r0, #0                  // Code of Success Write
                movt            r3, #4                  // r3 = 0x401D0
                                                        // bit[4] ready/stop 0b1 = stop
                                                        // bits[6-7] P2WI Cycles 0b11 = 3 Wait Cycle
                                                        // bit[8] P2WI mode 0b0 = Burst, 0b1 = single
                                                        // bit[18] P2WI Enable/Disable   0b0 = Disable 0b1 = Enable
...
.P2WI_0_Write:
                ...
                mov             r0, #0
                pop             {r1-r5, pc}
// End of function ATC2603C_PMIC_Register_Write


// =============== S U B R O U T I N E =======================================
// Handshake Bus
// This Function checks the aknowledge bit making sure data sent correctly.
// input:
// r0: Data to be sent over
// r1: P2WI_Bus_Selector 0 = P2WI_0 1 = P2WI_1
// ===========================================================================
PMIC_Bus_Sync:
                cmp             r1, #0
                bne             .P2WI_1_Bus_Select
                ldr             r1, =P2WI_0_Base
                ldr             r3, =0x800000               // Iterations
                mov             r2, r1
                str             r0, [r1, #P2WI_DAT_OUT]     // 0 = No Data Written xx = Data To Write/ Register Address
                b               .start_Sync0
// ---------------------------------------------------------------------------

.loop_Sync0:
                ldr             r12, [r2,#P2WI_STAT]
                ands            r0, r12, #4
                bne             .end_Sync0
                subs            r3, r3, #1
                beq             .end_Sync0
...
.start_Sync0:
                ldr             r0, [r2, #P2WI_STAT]
                sub             r3, r3, #1
                ands            r0, r0, #4                  // Check For Word State
                beq             .loop_Sync0
...
.P2WI_1_Bus_Select:
                ...
                ldr             r3, =0x800000               // Iterations
                mov             r2, r1
                str             r0, [r1, #P2WI_DAT_OUT]     //0 = No Data Written xx = Data To Write
                b               .start_Sync1
// ---------------------------------------------------------------------------
...
checkButtons:
    ...
    mov     r0, #PMU_REMCONADC
    bl      ATC2603C_PMIC_Register_Read     // r0= 0x3E2:No Press 0xC4: Vol Up Pressed 0x129: Vol Down Pressed
    lsr     r0, r0, #4
    and     r0, r0, #0xFF            // Get bits[0-4]
    cmp     r0, #0x12                // vol -
    bne     .volUp
...
.checkSelectSw:
    ...
    lsr     r0, r0, #15             // Shift bit[15] State to far right
...
.waitToRelaeseoPower:
    bl      PWR_SW_Read
    lsr     r0, r0, #15             // Shift bit[15] State to far right
    and     r0, r0, #0b1            // Get bits[0]
    cmp     r0, #0
    bne     .waitToRelaeseoPower
...
// =============== S U B R O U T I N E =======================================
// Cover Splash With A Pink Screen
// ===========================================================================
pinkout:
    push    {r0-r4, lr}
...
// ===========================================================================
 draw_point_normal:
    ...
    ldr     r0, =LCD_BASE               // FrameBufferBase 0x7FB80000
    ...
    pop     {r0-r7,pc}
    .ltorg
 // End of function draw_point
 
 
// =============== S U B R O U T I N E =======================================
// Convert Offset Aka X, Y Point into Address
// Handles 180-degree rotation (0,0 at Lower Right)
// Address = FrameBufferBase + (Bpp * [(width * Y) + X])
// input:
// r2 = X
// r3 = Y
...
// ===========================================================================
 draw_point_180:
    ...
    ldr     r0, =LCD_BASE               // FrameBufferBase 0x7FB80000
    mov     r1, #GFX_WIDTH              // LCD Width 800
    mov     r7, #GFX_HEIGHT             // LCD Height 480
    mov     r5, #2                      // Bpp = 2 (16 bit color)

    @ --- Tweak for 180-degree rotation ---
    @ flip X: r2 = (800 - 1) - r2
    sub     r6, r1, #1
    sub     r2, r6, r2
...
// =============== S U B R O U T I N E =======================================
// Convert Offset Aka X, Y Point into Address
// Handles Off the Range Drawing like getting out of the 800 x 480
// Handles 180-degree rotation (0,0 at Lower Right)
// Address = FrameBufferBase + (Bpp * [(width * Y) + X])
// input:
// r2 = X
...
draw_point:
    ...
    @ --- STEP 0: Immediate Boundary Check (Safety Railings) ---
    cmp     r2, #GFX_WIDTH          @ Is X >= 800? (includes negative check via unsigned 'hs')
    bhs     .skip_pixel             @ If out of bounds, exit immediately
    cmp     r3, #GFX_HEIGHT         @ Is Y >= 480?
    bhs     .skip_pixel

    ldr     r0, =LCD_BASE           @ 0x7FB80000
    mov     r1, #GFX_WIDTH          @ GFX_WIDTH
    mov     r7, #GFX_HEIGHT         @ GFX_HEIGHT
    ...
    @ --- Address Calculation ---
    mul     r6, r1, r3      @ r6 = 800 * Y
    add     r6, r2, r6      @ r6 = (800 * Y) + X
    lsl     r6, r6, #1      @ r6 = r6 * 2 (Bpp=2 is faster with shift)
    
    ldr     r0, =LCD_BASE
    strh    r4, [r0, r6]    @ Write the 16-bit color
...
//--------------------------------------------------------------------------------
@ 1st Flip X-axis (important for algorithm cause screen is 180º (0,0) at lower right
//--------------------------------------------------------------------------------
    mov     r11, #GFX_WIDTH
    sub     r11, r11, #1      @ r4 = 799 (or Width-1)
    sub     r2, r11, r2      @ New X0
    sub     r6, r11, r6      @ New X1

    @ Flip Y-axis
    mov     r11, #GFX_HEIGHT
    sub     r11, r11, #1      @ r4 = 479 (or Height-1)
    sub     r3, r11, r3      @ New Y0
    sub     r7, r11, r7      @ New Y1
//--------------------------------------------------------------------------------
...
.region2:
 ...
 .l0:
    ...
    ldr     r3, [r3]
//--------------------------------------------------------------------------------
@ 2nd Flip X-axis (important for algorithm cause screen is 180º (0,0) at lower right
//--------------------------------------------------------------------------------
    mov     r11, #GFX_WIDTH
    sub     r11, r11, #1      @ r4 = 799 (or Width-1)
    sub     r2, r11, r2      @ New X
...
//--------------------------------------------------------------------------------
    ...
    sub     r11, r11, #1      @ r4 = 799 (or Width-1)
    sub     r2, r11, r2      @ New X0
    sub     r0, r11, r0      @ New X1

    @ Flip Y-axis
    mov     r11, #GFX_HEIGHT
    sub     r11, r11, #1      @ r4 = 479 (or Height-1)
    sub     r3, r11, r3      @ New Y0
    sub     r1, r11, r1      @ New Y1
//--------------------------------------------------------------------------------
...
RTC_Read:
    ...
    mov     r0, #RTC_MS
    bl      ATC2603C_PMIC_Register_Read
    and     r2, r0, #0x3F                   // Get only bits[0-5] 6 bits Only
    mov     r8, r2                          // Save Seconds bits[0-5] (bin size 6 bits) r8 = sec
    
    // -- Get Minutes --
    lsr     r0, r0, #6                      // Get Minutes only bits[6-11]
    and     r0, r0, #0x3F                   // Get only bits[0-5] 6 bits Only
    ...
    mov     r0, #RTC_H
    bl      ATC2603C_PMIC_Register_Read
    and     r0, r0, #0x1F                   // Get only bits[0-4] 5 bits only
    ...
    mov     r0, #RTC_DC
    bl      ATC2603C_PMIC_Register_Read
    lsr     r0, r0, #7                      // Get Week Days only bits[7-9]
    and     r0, r0, #0x7                    // Get Bits[0-2] has 3 bits only
    ...
    mov     r0, #RTC_YMD
    bl      ATC2603C_PMIC_Register_Read
    and     r2, r0, #0x1F                   // Get Bits[0-4] has 5 bits only
    mov     r9, r2                          // Save Days bits[0-4] (bin size 5 bits) r9 = Days

    // -- Get Months --
    lsr     r2, r0, #5                      // Get Months only bits[5-8]
    and     r2, r2, #0xF                    // Get Bits[0-3] has 4 bits only
    lsl     r2, r2, #8                      // Shift To the Location Of Months bits[8-11] (bin size 4 bits)
    orr     r9, r2, r9                      // Save Months (r9 = Months | Days)
        
    // -- Get Years --
    lsr     r0, r0, #9                      // Get Years only bits[9-15]
    and     r0, r0, #0x7F                   // Get Bits[0-6] has 7 bits
...
RTC_Write:
    ...
    mov     r0, #RTC_MS
    bl      ATC2603C_PMIC_Register_Read
    bic     r0, r0, #0x3F                   // Clear bits[0-5] keep Minutes bits[6-11] Only
    orr     r1, r0, r8                      // Mix Values stored with the Data
    mov     r0, #RTC_MS
    bl      ATC2603C_PMIC_Register_Write
    b       .rtc_write_end
...
.chkMin:
    ...
    mov     r0, #RTC_MS
    bl      ATC2603C_PMIC_Register_Read
    lsl     r0, r0, #26                     // 26 = 31 - 5
    lsr     r0, r0, #26                     // Clear bits[6-31] keep seconds bits[0-5] Only
    lsl     r8, r8, #6                      // Shift Data to bits[6-11] Minutes
    orr     r1, r0, r8                      // Mix Values stored with the Data
    mov     r0, #RTC_MS
    bl      ATC2603C_PMIC_Register_Write
    b       .rtc_write_end
...
.chkDay:
    ...
    mov     r0, #RTC_YMD
    bl      ATC2603C_PMIC_Register_Read
    bic     r0, r0, #0x1F                   // Clear bits[0-4] keep Year/Mon bits[5-15] Only
    orr     r1, r0, r8                      // Mix Values stored with the Data
    mov     r0, #RTC_YMD
    bl      ATC2603C_PMIC_Register_Write
    b       .rtc_write_end
...
.chkWek:
    ...
    mov     r0, #RTC_DC
    bl      ATC2603C_PMIC_Register_Read
    lsl     r0, r0, #25                     // 25 = 31 - 6
    lsr     r0, r0, #25                     // Clear bits[7-31] keep Century bits[0-6] Only
    lsl     r8, r8, #7                      // Shift Data to bits[7-9] Week Day
    orr     r1, r0, r8                      // Mix Values stored with the Data
    mov     r0, #RTC_DC
    bl      ATC2603C_PMIC_Register_Write
    b       .rtc_write_end
...
.chkMon:
    ...
    bl      ATC2603C_PMIC_Register_Read
    mov     r2, #0x1E0
    bic     r0, r0, r2                      // Clear bits[5-8] keep Year/Day bits[9-15]/bits[0-4] Only
    lsl     r8, r8, #5                      // Shift Data to bits[5-8] Month
    orr     r1, r0, r8                      // Mix Values stored with the Data
    mov     r0, #RTC_YMD
    bl      ATC2603C_PMIC_Register_Write
    b       .rtc_write_end
...
.chkYer:
    ...
    mov     r0, #RTC_YMD
    bl      ATC2603C_PMIC_Register_Read
    lsl     r0, r0, #23                     // 23 = 31 -8
    lsr     r0, r0, #23                     // Clear bits[9-31] keep Mon/Day bits[0-8] Only
    lsl     r8, r8, #9                      // Shift Data to bits[5-8] Year
    orr     r1, r0, r8                      // Mix Values stored with the Data
    mov     r0, #RTC_YMD
    bl      ATC2603C_PMIC_Register_Write
    b       .rtc_write_end
...
// Input:
// Output:
// r0 = Power Switch Condition bit[15]
//      0b0: Released
//      0b1: Click
//========================================================================
PWR_SW_Read:
...
Battery_Fuel_Read:
    ...
    @ Input: r0 = ADC Value (540 to 713)
    @ Output: r0 = 0x0 to 0x64 (0 to 100 decimal)

    movw    r1, #540        @ Set the minimum anchor
    subs    r0, r0, r1      @ r0 = ADC - 540
    movmi   r0, #0          @ Clamp: If result is negative, set to 0
    
    @ Fixed-point multiply to avoid division:
    @ (100 / 180) * 256 is approx 142.2. We use 142.
    mov     r1, #142
    mul     r0, r0, r1      @ r0 = (ADC-540) * 148
    lsr     r0, r0, #8      @ r0 = r0 / 256 (Shift right 8 bits)

    @ Final Clamp
    cmp     r0, #100
    movhi   r0, #100        @ If > 100, set to 100
...
.ATCletsdoitagain_h:
    ...
    cmp     r1, #1
    ldreq   r9, =0x80000000
    ldrne   r9, =0x100                  // count of loops before checking next page 0x800 = around 10 sec
    mov     r2, r9
...
.ATCdoNotSave_h:
    ...
    subs    r9, r9, #1
    bne     .ATCfromstart_h
    add     r6, r6, #0x30                     // add next page (48)
    b       .ATCletsdoitagain_h

    pop {r0-r10, pc}
//========================================================================
// ATC2603dumpleastHexMem
...
// next page comes after 10 seconds and so on
// Input
//  r0 = Address of the ATC2603 Register to start from
//  r1: loop/stop state 0 = loop 1 = stop
// Output:
//========================================================================
ATC2603dumpleastHexMem:
...
.letsdoitagain_h:
    ...
    cmp     r1, #1
    ldreq   r9, =0x80000000
    ldrne   r9, =0x20                  // count of loops before checking next page 0x800 = around 10 sec
    mov     r2, r9
                                        // to stop the flip pages put this to 0x80000 useful to check single page
.fromstart_h:
    mov     r4, #20
...
delay_1s:
    ...
    ldr     r0, =0x10000     @ adjust experimentally
